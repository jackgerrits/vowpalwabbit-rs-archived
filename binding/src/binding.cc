...
DLL_PUBLIC int VWInitializeWorkspace(
    const char* const* symbols, int count, VWWorkspace** output_handle, const char** error_message) VW_RS_API_NOEXCEPT
{
  try
  {
...
DLL_PUBLIC int VWFreeWorkspace(VWWorkspace* workspace_handle, const char** error_message) VW_RS_API_NOEXCEPT
{
...
DLL_PUBLIC int VWRunDriver(VWWorkspace* workspace_handle, const char** error_message) VW_RS_API_NOEXCEPT
{
  auto& workspace = *reinterpret_cast<VW::workspace*>(workspace_handle);
